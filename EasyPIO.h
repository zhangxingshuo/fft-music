...
void myPinMode(int pin, int function) {
    int reg      =  pin/10;
    int offset   = (pin%10)*3;
...
void digitalWrites(int pins[], int numPins, int val) {
    int i;
    for(i=0; i<numPins; i++) {
        myDigitalWrite(pins[i], (val & 0x00000001));
        val = val >> 1;
    }
}

int digitalReads(int pins[], int numPins) {
    int i, val = myDigitalRead(pins[0]);
    
    for(i=1; i<numPins; i++) {
        val |= (myDigitalRead(pins[i]) << i);
    }
    return val;
}
